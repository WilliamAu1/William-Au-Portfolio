...
ggplot(tmp, aes(x = rep_dt, y = rep_num)) + geom_line() + 
    ...
    labs(title = "Top 2 most suspicious FSA neighbourhoods")
tmp <- f1 %>%
    right_join(hit_list_fsa_sector, by = c("pc", "sector"))
ggplot(tmp, aes(x = rep_dt, y = rep_num)) + geom_line() + 
    theme_void() + 
    labs(title = "Top 1 most suspicious neighbourhood-sector combination")
```