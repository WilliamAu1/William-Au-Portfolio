...
pg01 <- as_tibble(read_csv("../input/pokemongo/pokemonGO.csv", 
        col_names = TRUE)) %>%
...
pg02 <- as_tibble(read_csv(
        "../input/pokemon-species-stats/TDO SHEET BY DPS - Sheet1.csv", 
        col_names = TRUE)) %>%
...
pg04 <- pg01 %>%
    ...
    select(pm_nbr, name, typ1, typ2, max_cp, max_hp, weave_dps, tdo, tdo_cp, 
        surv_time) %>%
    bind_cols(pg03) %>%
    select(-max_cp1, -max_hp1) %>%
